...
class chat_settings:
	...
	def show_settings(event, chatSettings): #вывод настроек беседы
		if str(event.chat_id) in chatSettings:
			...
			result += '🔹Новости о боте:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["update_news"]) + '\n'
			result += '🔹️Упоминания в приветствиях:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["mention_in_greetings"]) + '\n'
			result += '🔹Оповещения о новых уровнях:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["lvlup_mentions"]) + '\n'
			result += '🔹Кто может кикать пользователей:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["kick_access"]) + '\n'
			result += '🔹Кто может банить пользователей:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["ban_access"]) + '\n'
			result += '🔹Кто может выдавать предупреждения:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["warn_access"]) + '\n'
			result += '🔹Ролевые команды:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["rp"]) + '\n'
			result += '🔹Кто может управлять ролевыми командами:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["rp_access"]) + '\n'
			result += '🔹Использование ролевых команд ботом:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["bot_rp"]) + '\n'
			result += '🔹Никнеймы:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["nicknames"]) + '\n'
			result += '🔹Кто может менять чужие никнеймы:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["nicknames_access"]) + '\n'
			result += '🔹Наказание за лимит предупреждений:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["warn_punishment"]) + '\n'
			result += '🔹Лимит предупреждений:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["warn_limit"]) + '\n'
			result += '🔹Реакция на простые команды\n(Привет, спокойной ночи и т.д.):\n' + bot_settings.optionValue_visual(settings["chat_settings"]["easy_commands_react"]) + '\n'
			result += '🔹Автокик:\n' + bot_settings.optionValue_visual(settings["chat_settings"]["autokick"]) + '\n'
			return result
		else:
			raise darkyExceptions.DarkyError(darkyExceptions.get_error(101))
